...
- 在草稿提交前，必须对每篇 `article-*.md` 的 Front Matter 和 `issue.yml` 执行真实 YAML 解析校验；仅目测格式不算通过。
- 在正式发布提交前，必须再次对将写入 `content/` 的全部 YAML/Front Matter 执行真实 YAML 解析校验。
- 包含冒号后空格、`#`、方括号、花括号或其他可能被 YAML 解释为语法的普通字符串时，应使用引号，尤其是来源标题。
- 任一文件无法被 `yaml.safe_load` 等标准 YAML 解析器成功解析时，不得报告草稿生成成功或正式发布成功；应先修复并重新解析。
